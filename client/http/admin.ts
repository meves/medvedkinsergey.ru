...
export const receiveClientMessages = async (page: number, count: number) => {
    const response = await fetch(`${process.env.url_api}/admin/client-messages?page=${page}&count=${count}`, {
        method: 'GET',
...
export const updateClientMessageChecked = async (id: number, checked: boolean) => {
    const response = await fetch(`${process.env.url_api}/admin/client-messages`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
